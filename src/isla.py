...
def generar_mapa() -> list:
    ...
    mapa[2][2] = CASILLA_JUGADOR

    # Colocar pistas y trampas
    for i in range(0,DIMENSIONES):
        for j in range(0, DIMENSIONES):
            if mapa[i][j] != CELDA_TESORO and mapa[i][j]!= CASILLA_JUGADOR: #Elimina la posibilidad de que aparezca o una trampa o una pista en el lugar del jugador(SIN TERMINAR).
...
def pedir_movimiento(mapa: list) -> str:
    ...
    entrada = str(input("Ingresa tu movimiento (formato: 'u:arriba', 'd:abajo', 'l:izquierda', 'r:derecha', q:salir): ")).lower() #Tambien se podrán introducir mayusculas
    while not entrada_correcta:
        ...
        if not entrada_correcta:
            entrada = str(input(
                "Ingresa tu movimiento (formato: 'u:arriba', 'd:abajo', 'l:izquierda', 'r:derecha', q:salir): ")).lower()
...
def simbolo_celda(celda):
    ...
    while celda != posicion_inicial_del_jugador():
...
def imprimir_mapa_oculto(mapa: list):
    ...
    col = 1
    for fila in mapa:
        print(str(col)+"|"," ".join([simbolo_celda(celda) for celda in fila]))
        col += 1
    print("-"*13)
...
def imprimir_mapa(mapa: list):
    ...
    col = 1
    for fila in mapa:
        print(str(col)+"|"," ".join([celda for celda in fila]))
        col += 1
    print("-"*13)



def muestra_resultado_del_movimiento(resultado_movimiento: int, nueva_posicion: tuple, mapa: list):
    """
    ...
    """
    if resultado_movimiento == MOVIMIENTO_INVALIDO:
        borrarconsola()
        print("Movimiento inválido. Estás intentando salir del mapa.")
    elif resultado_movimiento == TESORO_ENCONTRADO:
        print("¡Has encontrado el tesoro y ganado el juego!")
    elif resultado_movimiento == TRAMPA_ENCONTRADA:
        borrarconsola()
        print("Es una trampa. Intenta de nuevo.")
    elif resultado_movimiento == PISTA_ENCONTRADA:
        borrarconsola()
        pista = mapa[nueva_posicion[FILAS]][nueva_posicion[COLUMNAS]]
        print(f"Hay una pista!!!! La pista es: {pista}")
    elif resultado_movimiento == VACIA_ENCONTRADA:
        borrarconsola()

def posicionRelativa(posicion_jugador) -> tuple:
    # Cambia las coordenadas que va a ver el usuario. (+1 a ambas coordenadas)
    posicionR=list()
    posicionR.append(posicion_jugador[0]+1)
    posicionR.append(posicion_jugador[1]+1)
    return posicionR
...
def muestra_estado_mapa(mapa, posicion_jugador):
    ...
    print(f"Tu posición es {posicionRelativa(posicion_jugador)}")
